...
fn main() -> Result<()> {
    ...
    let public_key = PKey::from_rsa(private_key).unwrap().rsa().unwrap();
    let _domain_key = Rsa::generate(4096).unwrap();

    let public_key_str =
        String::from_utf8(public_key.private_key_to_pem().unwrap()).unwrap();
...
";
    ...
    fn public_key_modulus() -> Result<()> {
        ...
        // let modulus_base64 =
        //     openssl::base64::encode_block(&public_key.n().to_vec())
        //         .replace("=", "");

        let modulus_base64 = b64::b64u_encode(public_key.n().to_vec());

        // println!("modulus_base64: {modulus_base64}");
        assert_eq!(FIXTURE_MODULUS_BASE64_URL_NOPAD, modulus_base64);
    ...
    fn public_key_exponent() -> Result<()> {
        ...
        let exponent_base64 = b64::b64u_encode(public_key.e().to_vec());

        println!("exponent_base64: {exponent_base64}");
        assert_eq!(FIXTURE_EXPONENT_BASE64_URL_NOPAD, exponent_base64);
